...
class DomainNameAnalysis(models.Model):
    ...
    version = models.PositiveSmallIntegerField(default=16)
    ...
    #XXX hack to include comma-delimited list of auth_servers, prior to ns_mappings being available
    servers_in_child = models.CharField(max_length=4096)
    ...
    def name_for_ip(self, ip):
        ...
        #XXX hack for where auth_ips doesn't yet exist in previous versions
        if self.version < 6 and ip in self.get_servers_in_child().union(self.get_servers_in_parent()):
            return [], self.name
    ...
    def get_servers_in_child(self):
        if not hasattr(self, '_servers_in_child') or self._servers_in_child is None:
            #XXX hack for where auth_ips doesn't yet exist in previous versions
            if self.version < 6:
                servers = set(self.servers_in_child.split(','))
            else:
                servers = set()
                auth_ips = self.get_auth_ip_mapping()
...
class HistoricalNSMapperManager(models.Manager):
    def latest(self, name, date=None):
        f = Q(name=name)
        if date is not None:
            f &= Q(date__lte=date)
...
class HistoricalNSMapper(models.Model):
    name = DomainNameField(max_length=2048, canonicalize=False)
    rdtype = UnsignedSmallIntegerField()
    addresses = models.CharField(max_length=16000)
    date = models.DateTimeField()

    objects = HistoricalNSMapperManager()
