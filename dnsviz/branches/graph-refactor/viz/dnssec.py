...
if (typeof String.prototype.trim !== 'function') {
    ...
    def add_dnskey(self, name_obj, dnskey, ttl, trust_anchor, servers_with_dnskey, servers_without_dnskey):
        ...
        if not self.G.has_node(node_str):
            ...
            if dnskey.flags & util.format.DNSKEY_FLAGS['revoke']:
                ...
                attr['peripheries'] = '2'
                self.ta_algs[zone_top_name].add(dnskey.algorithm)
    ...
    def add_ds(self, name, ds, rdtype, ttl, zone_obj, parent_obj, servers_with_ds, servers_without_ds):
        ...
        if not self.G.has_node(node_str):
            ...
            if ds[0].algorithm in self.dnssec_algorithms and \
                ...
                self.ds_algs[(zone_top_name, parent_top_name)].add(ds[0].algorithm)
    ...
    def graph_zone_auth(self, name_obj, is_dlv):
        ...
        for dlv in False, True:
            ...
            if ds_rrsets_rrsigs and filter(lambda x: x[0][0].rdtype == rdtype, ds_rrsets_rrsigs):
                ...
                ds_algs = self.ds_algs.get((zone_top, parent_top), set())
    ...
    def add_trust(self, dlv_name):
        ...
        #XXX should this include trusted_keys for which no algorithm is known?
        trusted_zone_top_names = set([self.get_zone(z)[3] for z in trusted_keys if ])
        #zones_with_trusted_ds = set()
        for zone, dnskeys in trusted_keys.items() + dlv_tuple_extension:
            for dnskey in dnskeys:
                try:
                    dnskey_node = self.get_dnskey(self.id_for_dnskey(zone, dnskey), zone, dnskey.algorithm, util.dnssec.key_tag(dnskey))
                    self._add_trust_to_nodes_in_chain(dnskey_node, dlv_name, trusted_zone_top_names, zones_with_trusted_ds, [])
                except KeyError:
                    pass
    ...
    def _has_valid_self_loop(self, n):
        for e in self.G.out_edges(n):
            if (n,n) == e and e.attr['color'] == COLORS['secure']:
                return True
    ...
    def _add_trust_to_nodes_in_chain(self, n, trusted_zones, zones_with_trusted_ds, trace):
        if n in trace:
            return

        is_dlv = n.startswith('DLV-')
        is_ds = n.startswith('DS-') or is_dlv
        ...
        if is_trust_anchor:
            #XXX do we want this? check resolver behavior
            #if not self.ta_algs[top_name].intersection(self.dnssec_algorithms):
            #    top_name.attr['color'] = COLORS['insecure']
            #    return
            if self._has_valid_self_loop(n):
        ...
        # revoked DNSKEY must be self-signed
        #XXX revisit if we want to do this here
        if is_revoked:
            if not self._has_valid_self_loop(n):
        ...
        for e in self.G.in_edges(n):
            ...
            if is_ds:
                # if this zone has a trust anchor, then
                # don't handle it
                if prev_top_name in trusted_zones:
                    continue

                # if this is a DS record proper, then mark
                # the zone as having a DS
                if not is_dlv:
                    zones_with_trusted_ds.add(prev_top_name)

                # if this is a DLV, and the zone has a DS
                # then don't handle it
                elif prev_top_name in zones_with_trusted_ds:
                    continue

                # if the DNSSEC algorithm isn't understood,
                # then mark it as insecure
                if not self.ds_algs[(prev_top_name, top_name)]:
                    if prev_top_name.attr['color'] not != COLORS['secure']:
            ...
            # if the color of the edge isn't secure, then don't follow it
            if e.attr['color'] != COLORS['secure']:
                continue

            # if this is a DS, and the corresponding DNSKEY is self-signed,
            # then mark the child zone as secure
            if is_ds and self._has_valid_self_loop(p):
            ...
            self._add_trust_to_nodes_in_chain(p, dlv_name, trusted_zones, zones_with_trusted_ds, trace+[n])
